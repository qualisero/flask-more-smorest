"""Test User.has_domain_access method."""

import uuid

from flask_more_smorest.perms.models import User, UserRole
...
def test_has_domain_access_with_specific_domain(unit_app, db_session):
    ...
    role = UserRole(user_id=user.id, role="user", domain_id=domain_id)
    sqla_db.session.add(role)
    sqla_db.session.commit()

    # User should have access to their domain
    assert user.has_domain_access(domain_id) is True

    # User should not have access to other domain
...
def test_has_domain_access_with_none(unit_app, db_session):
    ...
    # User without roles should still have access to None
...
def test_has_domain_access_with_wildcard(unit_app, db_session):
    ...
    role = UserRole(user_id=user.id, role="admin", domain_id=None)
    sqla_db.session.add(role)
    sqla_db.session.commit()

    # User should have access to any domain
    domain_id = uuid.uuid4()
    assert user.has_domain_access(domain_id) is True
    assert user.has_domain_access(None) is True
...
def test_has_domain_access_multiple_domains(unit_app, db_session):
    ...
    role1 = UserRole(user_id=user.id, role="user", domain_id=domain1_id)
    role2 = UserRole(user_id=user.id, role="admin", domain_id=domain2_id)
    sqla_db.session.add_all([role1, role2])
    sqla_db.session.commit()

    # User should have access to both domains
...
def test_has_domain_access_no_roles(unit_app, db_session):
    ...
    # User without roles has no domain access (except None)
    domain_id = uuid.uuid4()
...
def test_domain_ids_property(unit_app, db_session):
    ...
    role1 = UserRole(user_id=user.id, role="user", domain_id=domain1_id)
    role2 = UserRole(user_id=user.id, role="admin", domain_id=domain2_id)
    role3 = UserRole(user_id=user.id, role="superadmin", domain_id=None)  # Wildcard
