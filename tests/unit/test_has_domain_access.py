"""Test defaults_module.DefaultUser.has_domain_access method."""
...
def test_has_domain_access_with_specific_domain(unit_app, db_session):
    ...
    role = defaults_module.DefaultUserRole(user_id=user.id, role="user", domain_id=domain_id)
    sqla_db.session.add(role)
    sqla_db.session.commit()

    # defaults_module.DefaultUser should have access to their domain
    assert user.has_domain_access(domain_id) is True

    # defaults_module.DefaultUser should not have access to other domain
...
def test_has_domain_access_with_none(unit_app, db_session):
    ...
    # defaults_module.DefaultUser without roles should still have access to None
...
def test_has_domain_access_with_wildcard(unit_app, db_session):
    ...
    role = defaults_module.DefaultUserRole(user_id=user.id, role="admin", domain_id=None)
    sqla_db.session.add(role)
    sqla_db.session.commit()

    # defaults_module.DefaultUser should have access to any domain
    domain_id = uuid.uuid4()
    assert user.has_domain_access(domain_id) is True
    assert user.has_domain_access(None) is True
...
def test_has_domain_access_multiple_domains(unit_app, db_session):
    ...
    role1 = defaults_module.DefaultUserRole(user_id=user.id, role="user", domain_id=domain1_id)
    role2 = defaults_module.DefaultUserRole(user_id=user.id, role="admin", domain_id=domain2_id)
    sqla_db.session.add_all([role1, role2])
    sqla_db.session.commit()

    # defaults_module.DefaultUser should have access to both domains
...
def test_has_domain_access_no_roles(unit_app, db_session):
    ...
    # defaults_module.DefaultUser without roles has no domain access (except None)
    domain_id = uuid.uuid4()
...
def test_domain_ids_property(unit_app, db_session):
    ...
    role1 = defaults_module.DefaultUserRole(user_id=user.id, role="user", domain_id=domain1_id)
    role2 = defaults_module.DefaultUserRole(user_id=user.id, role="admin", domain_id=domain2_id)
    role3 = defaults_module.DefaultUserRole(user_id=user.id, role="superadmin", domain_id=None)  # Wildcard
