...
class TestUserCustomization:
    ...
    def test_multiple_inheritance_with_mixins(self, app):
        ...
        from flask_more_smorest.user import ProfileMixin, SoftDeleteMixin
...
class TestRealWorldScenarios:
    ...
    def test_multi_tenant_user_system(self, app):
        ...
        with app.app_context():
            ...
            # Create tenant users
            owner = TenantUser(
                email=f"owner_{suffix}@tenant1.com",
                tenant_id="tenant_1",
                tenant_role="owner"
            )
            owner.save()
            ...
            user = TenantUser(
                email=f"user_{suffix}@tenant1.com", 
                tenant_id="tenant_1",
                tenant_role="user"
            )
            user.save()
    ...
    def test_api_user_with_tokens(self, app):
        ...
        with app.app_context():
            ...
            # Create API user
            api_user = APIUser(
                email=f"api_{suffix}@service.com",
