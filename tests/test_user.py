...
from flask_more_smorest.user import User, UserRole, UserSetting, get_current_user, get_current_user_id
...
class TestUserModel:
    ...
    def test_user_has_role(self, app):
        ...
        with app.app_context():
            ...
            assert not user.has_role(UserRole.Role.USER)
            assert not user.has_role(UserRole.Role.ADMIN)
...
class TestUserRole:
    ...
    def test_user_role_creation(self, app):
        ...
        with app.app_context():
            ...
            assert role.role == UserRole.Role.EDITOR
            assert role.user == user
            
    def test_user_role_enum_values(self):
        """Test UserRole enum values."""
        assert UserRole.Role.SUPERADMIN == "superadmin"
        assert UserRole.Role.ADMIN == "admin"
        assert UserRole.Role.EDITOR == "editor"
        assert UserRole.Role.USER == "user"
